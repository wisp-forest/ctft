...
public class CustomBowItem extends ProjectileWeaponItem implements Vanishable {
    ...
    public CustomBowItem(float UseDurationMultiplier, float VelocityMultiplier, Item.Properties p_40660_) {
        super(p_40660_);
            ...
            if (!itemstack.isEmpty() || flag) {
                ...
                if (!((double)f < 0.1D)) {
                    ...
                    if (!pLevel.isClientSide) {
                        ...
                        abstractarrow.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, f * 3.0F * VelocityMultiplier, 1.0F);
                        if (f == 1.0F) {
    ...
    public float getPowerForTime(int pCharge) {
        float f = pCharge / (20.0F * UseDurationMultiplier);
        f = (f * f + f * 2) / 3.0F;
        if (f > 1.0F) {
            f = 1.0F;
        }
