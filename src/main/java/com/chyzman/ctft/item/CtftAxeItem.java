...
public class CtftAxeItem extends AxeItem {
    ...
    public Text getName(ItemStack stack) {
        if (stack.getNbt() != null && stack.getNbt().getString("material") != null) {
            ...
            if (id != null) {
                return (Text.translatable("item.ctft.axe", Registries.ITEM.containsId(id) ? Registries.ITEM.get(id).getName() : Registries.BLOCK.containsId(id) ? Registries.BLOCK.get(id).getName() : Registries.ENTITY_TYPE.containsId(id) ? Registries.ENTITY_TYPE.get(id).getName() : "???"));
            }
        }
