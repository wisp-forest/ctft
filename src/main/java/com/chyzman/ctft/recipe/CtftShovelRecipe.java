...
public class CtftShovelRecipe extends SpecialCraftingRecipe {
    public CtftShovelRecipe(Identifier id, CraftingRecipeCategory category) {
        super(id, category);
    }

    @Override
    public boolean matches(CraftingInventory inventory, World world) {
        boolean left_handle = inventory.getStack(3).isOf(Items.STICK) && inventory.getStack(6).isOf(Items.STICK);
        boolean mid_handle = inventory.getStack(4).isOf(Items.STICK) && inventory.getStack(7).isOf(Items.STICK);
        boolean right_handle = inventory.getStack(5).isOf(Items.STICK) && inventory.getStack(8).isOf(Items.STICK);
        ...
        boolean left = left_handle && left_empty;
        boolean mid = mid_handle && mid_empty;
        boolean right = right_handle && right_empty;
        return left || mid || right;
    }

    @Override
    public ItemStack craft(CraftingInventory inventory, DynamicRegistryManager registryManager) {
        ItemStack out = new ItemStack(CtftRegistry.CTFT_SHOVEL);
    ...
    public RecipeSerializer<?> getSerializer() {
        return CtftRecipeSerializers.CTFT_SHOVEL_RECIPE;
    }
